...
ALTER TABLE public.leads ADD COLUMN IF NOT EXISTS enrollment_date date;

-- Backfill: for leads already marked as matricula, use the earliest "enrolled" event date; fallback to updated_at
UPDATE public.leads l
SET enrollment_date = COALESCE(
  (
    SELECT (e.created_at AT TIME ZONE 'America/Sao_Paulo')::date
    FROM public.lead_events e
    WHERE e.lead_id = l.id AND e.event_type = 'enrolled'
    ORDER BY e.created_at ASC
    LIMIT 1
  ),
  l.updated_at::date
)
WHERE l.status = 'matricula' AND l.enrollment_date IS NULL;

CREATE INDEX IF NOT EXISTS idx_leads_enrollment_date ON public.leads (enrollment_date) WHERE status = 'matricula';

-- Update productivity_summary to count matriculas by enrollment_date (with fallback to updated_at for legacy rows)
...
BEGIN
  ...
  matr AS (
    ...
    WHERE status = 'matricula'
      AND COALESCE(enrollment_date, updated_at::date) >= _start
      AND COALESCE(enrollment_date, updated_at::date) <= _end
    GROUP BY owner_id
  ),
  ck AS (
