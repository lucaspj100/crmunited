...
ALTER TABLE public.leads
  ADD COLUMN IF NOT EXISTS interview_done_date date;

-- Backfill: usa a data do evento interview_done mais recente por lead
WITH done_evt AS (
  SELECT DISTINCT ON (lead_id)
    lead_id,
    (created_at AT TIME ZONE 'UTC')::date AS d
  FROM public.lead_events
  WHERE event_type = 'interview_done'
  ORDER BY lead_id, created_at DESC
)
UPDATE public.leads l
  SET interview_done_date = de.d
FROM done_evt de
WHERE l.id = de.lead_id
  AND l.interview_done_date IS NULL;

-- Fallback: leads em status entrevista_realizada sem evento -> usa interview_date
UPDATE public.leads
  SET interview_done_date = interview_date
WHERE status = 'entrevista_realizada'
  AND interview_done_date IS NULL
  AND interview_date IS NOT NULL;

CREATE INDEX IF NOT EXISTS leads_interview_done_date_idx
  ON public.leads (interview_done_date);
...
BEGIN
  ...
  entrev_evt AS (
    ...
    JOIN public.leads l ON l.id = e.lead_id
    WHERE (
      e.event_type IN ('interview_scheduled','interview_rescheduled')
      OR (e.event_type = 'status_change' AND e.metadata->>'to' = 'entrevista_marcada')
    )
    AND e.created_at >= start_ts AND e.created_at < end_ts
  ),
  entrev_fb AS (
    SELECT id AS lead_id, owner_id AS vid FROM public.leads
    WHERE interview_date >= _start AND interview_date <= _end
  ),
  entrev AS (
    SELECT vid, count(DISTINCT lead_id)::int AS n
    FROM (
      SELECT lead_id, vid FROM entrev_evt
      UNION
      SELECT lead_id, vid FROM entrev_fb
    ) u
    WHERE vid IS NOT NULL
    GROUP BY vid
  ),
  -- Entrevistas REALIZADAS: pela data real da realização (interview_done_date)
  entrev_real AS (
