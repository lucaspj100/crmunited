...
BEGIN
  ...
  -- Entrevistas MARCADAS: eventos do período OU interview_date no período (independente do status atual)
  entrev_evt AS (
    SELECT DISTINCT l.id AS lead_id, l.owner_id AS vid
    FROM public.lead_events e
    JOIN public.leads l ON l.id = e.lead_id
  ...
  -- Entrevistas REALIZADAS: eventos do período (não dependem do status atual)
  entrev_real AS (
    SELECT l.owner_id AS vid, count(DISTINCT l.id)::int AS n
    FROM public.lead_events e
    JOIN public.leads l ON l.id = e.lead_id
    WHERE (
      e.event_type = 'interview_done'
      OR (e.event_type = 'status_change' AND e.metadata->>'to' = 'entrevista_realizada')
    )
    AND e.created_at >= start_ts AND e.created_at < end_ts
    GROUP BY l.owner_id
  ),
  -- Matrículas: apenas por data real da matrícula (independente do status atual)
  matr AS (
    SELECT owner_id AS vid, count(*)::int AS n FROM public.leads
  ...
  -- Perdidos: eventos do período (não dependem do status atual)
  perd AS (
    SELECT l.owner_id AS vid, count(DISTINCT l.id)::int AS n
    FROM public.lead_events e
    JOIN public.leads l ON l.id = e.lead_id
