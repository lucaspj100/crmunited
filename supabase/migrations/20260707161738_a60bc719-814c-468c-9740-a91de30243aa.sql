...
BEGIN
  ...
  -- Entrevistas MARCADAS: exclusivamente pela data agendada (interview_date) no range.
  -- Cada lead conta no máximo 1 vez para a data. Evita duplicação com histórico de eventos.
  entrev AS (
...
-- Debug: lista os leads considerados como "Entrevistas Marcadas" para o range/vendedor
...
SET search_path TO 'public'
AS $$
  SELECT l.id, l.owner_id, l.name, l.interview_date, l.status::text
  FROM public.leads l
  WHERE l.interview_date IS NOT NULL
    AND l.interview_date >= _start
    AND l.interview_date <= _end
  ...
  ORDER BY l.interview_date, l.name;
$$;
