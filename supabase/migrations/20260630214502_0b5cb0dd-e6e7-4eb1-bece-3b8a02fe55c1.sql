...
BEGIN
  ...
  interessados AS (
    SELECT vendedor_responsavel_id AS vid, count(*)::int AS n FROM public.prospect_contacts
    WHERE status_prospeccao = 'Interessado' AND updated_at >= start_ts AND updated_at < end_ts
  ...
  matr AS (
    SELECT owner_id AS vid, count(*)::int AS n FROM public.leads
    WHERE status = 'matricula' AND updated_at >= start_ts AND updated_at < end_ts GROUP BY owner_id
  ),
  ck AS (
