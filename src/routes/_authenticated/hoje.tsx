...
function HojePage() {
  ...
  const queue = useMemo<QueueItem[]>(
    () => buildTodayQueue(data, { userId: user?.id, isAdmin, vendor }),
    [data, user?.id, isAdmin, vendor],
  );
            ...
            <section key={reason} className="space-y-2">
              ...
              <div className="space-y-2">
                {items.slice(0, 50).map((it) => (
                  <Row key={it.key} item={it} isAdmin={isAdmin} owner={byProfile.get(it.owner_id)} onClick={() => openItem(it)} />
                ))}
              </div>
            </section>
          );
